...
<head>
  ...
  <title>Resi {{ $shipment->resi_no }} • Barcode</title>
  <style>
    ...
    .brand img { height: 90px; margin-right: 8px; }
    .brand h1 { margin:0; font-size: 26px; letter-spacing: .5px; font-weight: 800; }
    .org-lines { font-size: 10px; line-height: 1.4; margin-top: 0px; }
    .barcode { text-align:center; }
    .barcode .label { font-size: 11px; margin-top: 4px; }
    ...
    .barcode svg { width: 280px; height: 80px; }
  </style>
</head>
<body>
  <div class="header">
    ...
    <div class="barcode">
      <svg id="barcode"></svg>
      {{-- <div class="label">{{ $shipment->resi_no }}</div> --}}
    </div>
  </div>
  ...
  <br>
  <div class="small muted mb-10">Rute: {{ $shipment->origin?->city }} → {{ $shipment->destination?->city }} | Status: {{ $shipment->status }}</div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function(){
      ...
      var value = @json($shipment->resi_no ?? '');
      if (svg && value) {
        JsBarcode(svg, value, { format: 'code128', displayValue: true, fontSize: 12, height: 60, margin: 0 });
      }
    });
  </script>
</body>
